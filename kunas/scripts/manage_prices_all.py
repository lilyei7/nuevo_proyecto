#!/usr/bin/env python3
"""Interactive helper to manage price updates across all properties.

Features:
- prompt for credentials (or read/save ./credentials.txt)
- export properties to backups/
- apply a price update to all properties (with dry-run and automatic backups)

Usage: run without args for interactive prompts.
"""
import os
import sys
import json
...
def prompt_for_credentials():
    ...
    if creds.get('token') and creds.get('username') and creds.get('password'):
        ...
        if use == 'y':
            return creds
    print('Enter OTASync credentials (they will optionally be saved to ./credentials.txt)')
    token = input('Token: ').strip()
    username = input('Username: ').strip()
    password = getpass('Password: ')
    save = input('Save to credentials.txt? (y/N): ').strip().lower()
    if save == 'y':
...
def main():
    print('Manage prices helper')
    creds = prompt_for_credentials()
    while True:
        print('\nChoose an action:')
        print('  1) Export properties to backups/')
        print('  2) Apply price to ALL properties (safe: dry-run)')
        print('  3) Apply price to ALL properties (execute)')
        print('  4) Exit')
        ...
        if choice == '1':
            ...
            if confirm != 'y':
                ...
                continue
            apply_to_all_properties(creds, price, days=days, dry_run=dry)
        elif choice == '4':
            print('Bye')
            break
        else:
